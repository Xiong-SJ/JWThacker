...
class JWTcode:
    ...
    def examine_jwt(jwt_str:str,password:str|None=None) -> bool | None:
        ...
        try:
            header_dict  = json.loads(jwt_list[0])
            payload_dict = json.loads(jwt_list[1])
            token = JWTcode.jwt_encode(header_dict,payload_dict,password)
            if token is None:
                return None
            token = token.split('.')[-1]
...
def main():
    """主函数：程序入口逻辑"""
    secret_key = "YISHI"
    payload = {'p':1,"exp":1786387970,"username":"YISHI",'password':""}
    #
    # # 生成带HS256签名的令牌
    token = JWTcode.jwt_encode({"alg":'HS256',"typ":"JWT"},payload, "YISHI")
    print(token)
